...
module oracle_aggregator_pyth_lazer_rolling_integration::price_feed_storage;

use oracle_aggregator_pyth_lazer_rolling_integration::source::PYTH_LAZER_ROLLING;
use oracle_aggregator_pyth_lazer_rolling_integration::feed_info_object::FeedInfoObject;
...
use fun oracle_aggregator_pyth_lazer_rolling_integration::source::source_cap as Source.source_cap;
...
#[error(code = 0)]
const EFeedNotFound: vector<u8> =
    b"Pyth Lazer Rolling integration: the requested feed was not found.";

#[error(code = 1)]
const EPriceNotReported: vector<u8> =
    b"Pyth Lazer Rolling integration: the feed did not report a price.";

#[error(code = 2)]
const EExponentNotReported: vector<u8> =
    b"Pyth Lazer Rolling integration: the feed did not report an exponent.";

#[error(code = 3)]
const EPriceIsNegative: vector<u8> =
    b"Pyth Lazer Rolling integration: the price cannot be negative.";

#[error(code = 4)]
const EUnsupportedExponent: vector<u8> =
    b"Pyth Lazer Rolling integration: the feed's exponent is too large for the price to be representable.";

#[error(code = 5)]
const EPriceOverflow: vector<u8> =
    b"Pyth Lazer Rolling integration: the normalized price does not fit into a u128.";

#[error(code = 6)]
const EFeedUpdateTimestampNotReported: vector<u8> =
    b"Pyth Lazer Rolling integration: the feed did not report an update timestamp.";

//************************************************************************************************//
// Constants                                                                                      //
//************************************************************************************************//

macro fun one(): u256 { 1__000_000_000_000_000_000 }
...
use fun extract as Feed.extract;
fun extract(
    feed: &Feed,
    update: &Update,
...
use fun blended_price_and_timestamp_ms as FeedInfoObject.blended_price_and_timestamp_ms;
fun blended_price_and_timestamp_ms(
