...
module oracle_aggregator_pyth_lazer_integration::source;
...
public(package) fun source_cap(source: &Source<PYTH_LAZER>): &SourceCap {
    abort 404
}

public(package) fun borrow_mut_id(source: &mut Source<PYTH_LAZER>): &mut UID {
    abort 404
}
...
public(package) fun assert_version(source: &Source<PYTH_LAZER>) {
    abort 404
}
