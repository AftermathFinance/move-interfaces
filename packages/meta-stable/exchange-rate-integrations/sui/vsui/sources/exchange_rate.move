...
module meta_vault_vsui_integration::exchange_rate;
...
use liquid_staking::cert::{CERT as VSUI, Metadata};
use liquid_staking::native_pool::NativePool;

use sui::coin::CoinMetadata;

//************************************************************************************************//
// MetaVaultVSuiIntegration                                                                      //
//************************************************************************************************//
...
public struct MetaVaultVSuiIntegration has key, store {
...
/// Grant the `MetaVaultVSuiIntegration` object the ability to call `create_deposit_cap` and
/// `create_withdraw_cap`.
public fun authorize(
    admin_cap: &AdminCap,
    wrapper: &mut MetaVaultVSuiIntegration,
) {
    abort 404
}

/// Revoke the ability for the `MetaVaultVSuiIntegration` object to call `create_deposit_cap` and
/// `create_withdraw_cap`.
...
/// Get the exchange rate between a meta-coin and vSUI in 10^18 fixed point. This wrapper should
/// only be used for cases where the `MetaVault` is denominated in SUI, in which case the
/// meta-coin:vSUI exchange rate is equivalent to the SUI:vSUI exchange rate. This exchange rate
/// is queried directly from the `liquid_staking` package.
///
///  e.g.   1vSUI:1SUI = 1__000_000_000_000_000_000,
///       1vSUI:1.5SUI = 1__500_000_000_000_000_000.
///
/// Aborts:
...
public fun create_deposit_cap<MetaCoin>(
    wrapper: &MetaVaultVSuiIntegration,
    vault: &Vault<MetaCoin>,
    registry: &mut Registry,
): DepositCap<MetaCoin, VSUI> {
    abort 404
}
...
/// Query the `vSUI:SUI` exchange rate from the `vSUI` package and set it within the `Registry`.
///
/// Aborts:
///    i. [meta_vault::admin::ENotAuthorized]
...
public fun add_support_for_new_coin<MetaCoin>(
    ...
    max_fee: u64,
    priority: u64,
    coin_metadata: &CoinMetadata<VSUI>,
    registry: &mut Registry,
    staked_sui_vault: &NativePool,
    metadata: &Metadata<VSUI>,
    ctx: &mut TxContext,
...
use fun vsui_to_sui_exchange_rate as NativePool.vsui_to_sui_exchange_rate;
/// Return the exchange rate of `Coin<VSUI>` to `COIN<SUI>` normalized to 10^18; e.g., an exchange
/// rate of 10^18 means you can exchange `Coin<VSUI>` 1:1 to `COIN<SUI>`.
fun vsui_to_sui_exchange_rate(
    staked_sui_vault: &NativePool,
    metadata: &Metadata<VSUI>,
): u128 {
    abort 404
