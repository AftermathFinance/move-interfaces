...
module meta_vault_hasui_integration::exchange_rate;
...
public struct MetaVaultHaSuiIntegration has key, store {
...
/// Grant the `MetaVaultHaSuiIntegration` object the ability to call `create_deposit_cap` and
/// `create_withdraw_cap`.
public fun authorize(
    admin_cap: &AdminCap,
    wrapper: &mut MetaVaultHaSuiIntegration,
) {
    abort 404
}

/// Revoke the ability for the `MetaVaultHaSuiIntegration` object to call `create_deposit_cap` and
/// `create_withdraw_cap`.
...
/// Get the exchange rate between a meta-coin and haSUI in 10^18 fixed point. This wrapper should
/// only be used for cases where the `MetaVault` is denominated in SUI, in which case the
/// meta-coin:haSUI exchange rate is equivalent to the SUI:haSUI exchange rate. This exchange rate
/// is queried directly from the `liquid_staking` package.
///
///  e.g.   1haSUI:1SUI = 1__000_000_000_000_000_000,
///       1haSUI:1.5SUI = 1__500_000_000_000_000_000.
///
/// Aborts:
...
public fun create_deposit_cap<MetaCoin>(
    wrapper: &MetaVaultHaSuiIntegration,
    vault: &Vault<MetaCoin>,
    registry: &mut Registry,
): DepositCap<MetaCoin, HASUI> {
    abort 404
}
...
/// Query the `haSUI:SUI` exchange rate from the `haSUI` package and set it within the `Registry`.
///
/// Aborts:
///    i. [meta_vault::admin::ENotAuthorized]
...
public fun add_support_for_new_coin<MetaCoin>(
    ...
    max_fee: u64,
    priority: u64,
    coin_metadata: &CoinMetadata<HASUI>,
    registry: &mut Registry,
    staked_sui_vault: &Staking,
    ctx: &mut TxContext,
) {
...
/// Return the exchange rate of `Coin<HASUI>` to `COIN<SUI>` normalized to 10^18. e.g. an exchange
///  rate of 10^18 means you can exchange `Coin<HASUI>` 1:1 to `COIN<SUI>`.
fun hasui_to_sui_exchange_rate(
    staked_sui_vault: &Staking,
): u128 {
    abort 404
}
